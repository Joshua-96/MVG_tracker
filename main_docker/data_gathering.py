...
async def cache_dep(stationID,client):
    pending_tasks = [asyncio.ensure_future(get_dep(i,client)) for i in list(stationID.values())]
...
def merge_reduce_Df(inboundDf, outboundDf, suffixes, hour_axis):
    merged = inboundDf.merge(outboundDf,how="inner",left_on=["date","station",hour_axis],right_on=["date","station","hour"],suffixes=suffixes)
    del outboundDf
    merged = merged[merged[f"line{suffixes[0]}"]!=merged[f"line{suffixes[1]}"]]
    merged = merged[merged[f"timestamp{suffixes[0]}"]<merged[f"timestamp{suffixes[1]}"]]
    ...
    if f"timestamp_epoch{suffixes[1]}" in list(merged.columns):
        merged.drop(f"timestamp_epoch{suffixes[1]}",axis="columns",inplace=True)
...
class Logger:
    ...
    def log_errors(self,ErrorType: str,station="",line=""):
        ...
        Entry = {"ErrorType":[ErrorType], "time": [timeOfDay],"station": [station],"line":[line]}
...
class DataManager:
    ...
    def get_Df(self,cachedDep:dict,stationID:dict):
        depDf=pd.DataFrame(columns=[field.name for field in fields(departure)])
        for i,station in enumerate(stationID):
            querey= cachedDep[i]
            for dep in querey["departures"]:
                try:
                    if dep["product"]=="SBAHN" and (int(dep["departureTime"]/1000) > int(time.time()) - 3600) and "delay" in dep.keys():
                        timestamp_epoch = int(dep["departureTime"]/1000) + int(dep["delay"]*60)
                        cur_dep=departure(
                            timestamp_epoch = timestamp_epoch,
                            timestamp = datetime.fromtimestamp(timestamp= timestamp_epoch).strftime("%Y/%m/%d, %H:%M:%S"),
                            station=station,line=dep["label"],destination=dep["destination"],delay=dep["delay"],Id=dep["departureId"],
                            curr_time_epoch=int(datetime.timestamp(datetime.now()))
                        )
                        cur_depDf=pd.DataFrame(data=[cur_dep])
                        depDf= pd.concat([depDf, cur_depDf], ignore_index=True)

                except KeyError:
                    self.Logger.log_errors(ErrorType="KeyError",station=station,line=dep["label"])
                    continue
        return depDf
    ...
    def calculate_transfer(self,transDf):
        ...
        relevantStations = self.FileDict["transFile"].Bahnhof.drop_duplicates().values.tolist()
        transDf = transDf[transDf["station"].isin(relevantStations)]
        transDf.drop(["Id","curr_time_epoch"],axis="columns",inplace=True) 
        transDf=transDf.merge(self.FileDict["stationFile"],how="inner",on="station")
        transDf=transDf.merge(self.FileDict["destFile"],how="inner",on=["destination","Himmelsrichtung"])
        ...
        inbound_currDf = merge_reduce_Df(inboundDf, outboundDf, suffixes, "hour")

        inbound_nextDf = merge_reduce_Df(inboundDf, outboundDf, suffixes, "next_hour")
        
        combDf = pd.concat([inbound_currDf,inbound_nextDf])
        del inbound_currDf,inbound_nextDf
        combDf = combDf[combDf[f"timestamp{suffixes[0]}"]<combDf[f"timestamp{suffixes[1]}"]]
        combDf.sort_values(by=[f"timestamp{suffixes[0]}",f"timestamp{suffixes[1]}"],inplace=True)
        combDf.drop_duplicates([f"timestamp{suffixes[0]}","station",f"line{suffixes[0]}",f"line{suffixes[1]}"],keep="first",inplace=True)
    ...
    def load_trans_config(self):
        ...
        for name in self.assistFiles:
            ...
            self.FileDict[name]=pd.read_csv(loadPath,sep=";",index_col=None,encoding="cp1252")
        self.init_stationID()
        self.FileDict["stationFile"].drop(["Breitengrad","Längengrad","ID"],axis="columns",inplace=True)
    ...
    def update_db_table(self,Df):
        conn = self.get_connector()    
        baseDf= pd.read_sql(f'SELECT * FROM public."{self.depTableName}" WHERE timestamp_epoch >= {time.time()-self.timeOffset}',conn)
        conn.execute(f'DELETE FROM public."{self.depTableName}" WHERE timestamp_epoch >= {time.time()-self.timeOffset}')
        ...
        baseTransDf = pd.read_sql(f'SELECT * FROM public."{self.transTableName}" WHERE timestamp_epoch >= {time.time()-self.timeOffset}',conn)
        baseTransDf = pd.concat([baseTransDf, transTable], ignore_index=True)
        baseTransDf.drop_duplicates(["timestamp_from","station","line_from","line_to"],ignore_index=True,inplace=True)
        conn.execute(f'DELETE FROM public."{self.transTableName}" WHERE timestamp_epoch >= {time.time()-self.timeOffset}')
    ...
    async def main(self):
        ...
        while True:
            try:
                ...
                epochTime_Df=int(datetime.timestamp(
                                    datetime.strptime(
                                        currDf.sort_values("timestamp")[0:1].timestamp.values[0]
                                        ,"%Y/%m/%d, %H:%M:%S")
                                    )   
                                )
...
def get_json_from_path(Path):
    ...
    assert pl_Path.suffix==".json" , f"Incorrect Filetype given: {pl_Path.suffix} expected json"
